...
namespace CoreEngine
{
    ...
    void GameScene::OnUpdate() {
        ...
        if (gameResultManager_->CheckGameClear()) {
            ...
            // ゲームクリアシーケンスが完了してリザルトシーンへ遷移する時
            if (gameClearSequence_->IsReadyForTransition() && !isTransitioning_) {
                auto postEffectMgr = engine_->GetComponent<PostEffectManager>();
                if (postEffectMgr) {
                    auto transitionEffect = postEffectMgr->GetEffect<TransitionEffect>(PostEffectNames::TransitionEffect);
                    if (transitionEffect) {
                        transitionEffect->SetEnabled(true);
                        // リザルトシーンへの遷移はグリッチエフェクト
                        transitionEffect->SetTransitionType(TransitionEffect::TransitionType::Glitch);
                        transitionProgress_ = 0.0f;
                        isTransitioning_ = true;
                        nextSceneName_ = "ResultScene";
                    }
                }
            }
