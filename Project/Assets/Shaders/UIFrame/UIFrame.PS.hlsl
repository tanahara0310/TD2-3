// UIFrame ピクセルシェーダー（GLSLからHLSL変換）
struct PSInput
{
...
static const float3 OUTSIDE_COLOR = float3(0.0, 0.0, 0.0);  // 外側は黒

static const float MARGIN_PCT = 0.05;
...
float sdMainFrame(float2 p, float2 boxSize) 
{
    ...
    float bNotchW = gScene.resolution.y * NOTCH_WIDTH_RATIO;
    float bNotchH = gScene.resolution.y * NOTCH_HEIGHT_RATIO;
    float rNotchW = gScene.resolution.y * RIGHT_NOTCH_WIDTH_RATIO;
    float rNotchH = gScene.resolution.y * RIGHT_NOTCH_HEIGHT_RATIO;
    ...
    float tabH = gScene.resolution.y * TL_TAB_HEIGHT_RATIO;
    float tabW = gScene.resolution.y * TL_TAB_WIDTH_RATIO;
...
float2 snapToSurface(float2 pApprox, float2 boxSize) 
{
    float2 p = pApprox;
    for(int i = 0; i < 3; i++) 
    {
        float d = sdMainFrame(p, boxSize);
        float2 e = float2(0.01, 0.0);
        float2 g = normalize(float2(
            sdMainFrame(p + e.xy, boxSize) - sdMainFrame(p - e.xy, boxSize),
            sdMainFrame(p + e.yx, boxSize) - sdMainFrame(p - e.yx, boxSize)
        ));
        p -= g * d;
    }
    return p;
...
float getRotatingDiamondDist(float2 p, float2 center, float size) 
{
    float2 offset = p - center;
    float rotAng = gScene.time * DIAMOND_ROT_SPEED;
    float c = cos(rotAng);
    float s = sin(rotAng);
...
{
    // 解像度とピクセル座標の計算
    float2 iResolution = gScene.resolution;
    float iTime = gScene.time;
    ...
    // 形状ロジック
    float dFrame = sdMainFrame(p, boxSize);
    float dTabHard = sdTabShape(p, boxSize, tabH, tabW, CUT_SIZE);
    ...
    float2 posStart = snapToSurface(posStartApprox, boxSize);
    float2 posHead = snapToSurface(posHeadApprox, boxSize);
    ...
    if (tailPos < 0.1) 
    {
        float dStartDia = getRotatingDiamondDist(p, posStart, HEAD_DIAMOND_SIZE);
    ...
    {
        float dHeadDia = getRotatingDiamondDist(p, posHead, HEAD_DIAMOND_SIZE);
    ...
    {
        // 背景（外側）- 完全に透明にする
        finalResult = float4(0.0, 0.0, 0.0, 0.0);
    } 
    else 
    {
        // 内部（Land）- 不透明にする
        ...
        if (dTab > 0.0) 
        {
            ...
            col += BORDER_COLOR * 0.3 * gridAlpha;
        }
        
        col += totalDiamondLight;
        
        // 枠の部分にグロー効果を追加
        float3 borderGlow = baseColor * totalIntensity + float3(1.0, 1.0, 1.0) * electricSignal * glow * 0.3;
        col = max(col, borderGlow);
