#include "UIFrameRenderer.h"
#include "Engine/Camera/ICamera.h"
#include <cassert>

namespace CoreEngine
{
    void UIFrameRenderer::Initialize(ID3D12Device* device) {
        ...
        // シェーダーのコンパイルとPSO作成
        auto vertexShaderBlob = shaderCompiler_->CompileShader(L"Assets/Shaders/UIFrame/UIFrame.VS.hlsl", L"vs_6_0");
        assert(vertexShaderBlob != nullptr);

        auto pixelShaderBlob = shaderCompiler_->CompileShader(L"Assets/Shaders/UIFrame/UIFrame.PS.hlsl", L"ps_6_0");
        ...
        if (!result) {
            throw std::runtime_error("Failed to create UIFrame Pipeline State Object");
        }
    ...
    void UIFrameRenderer::BeginPass(ID3D12GraphicsCommandList* cmdList, BlendMode blendMode) {
        ...
        // カメラCBVを設定
        if (cameraCBV_ != 0) {
            cmdList->SetGraphicsRootConstantBufferView(UIFrameRendererRootParam::kScene, cameraCBV_);
        }
    }

    void UIFrameRenderer::EndPass() {
    }

    void UIFrameRenderer::SetCamera(const ICamera* camera) {
        if (camera) {
